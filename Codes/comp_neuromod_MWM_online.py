...
from NeuFun_Cuda import convolution, convolution_type2, neuron, weights_update_stdp, weights_update_rate
import csv
...
"""
Competitive weight change (CWC) implementation.
...
Options:
    ...
    - Relative reinforcement magnitude between reward and aversion (-r or --rew), this is R_D in methods.
    - Time for DA to be active (-x or --trew).
...
Run as: python comp_neuromod_MWM_online.py -o <name_job> -e <episodes> -t <trials> -s
...
def main():
    ...
    parser.add_option("-l", "--lr", dest="eta",
                  help="Learning rate for competitive reinforcement", metavar="float", default=0.0001)
    ...
    parser.add_option("-n", "--asero", dest="A_Sero",
                  help="STDP magnitude of serotonin", metavar="float", default=0.01)
    ...
    parser.add_option("-r", "--rew", dest="R_D",
                  help="Reward difference between punishment and reward", metavar="float", default=1)
    parser.add_option("-x", "--trew", dest="T_R",
                  help="Time for reward in dopamine to be active", metavar="int", default=1000)
    parser.add_option("-g", "--gtime", dest="TSero",
    ...
    parser.add_option("-k", "--randomloc", dest="randomloc",
                  help="Random initial locations", default=False, action='store_true')
    parser.add_option("-y", "--elegibilty", dest="tau_e_sero",
                  help="Elegibility trace constant for Sero",  metavar="float", default=5)
    ...
    #learning rates
    eta=float(options.eta) #learning rate for weight integral
    #magnitudes of neuromodulation
    A_DA=float(options.A_Dopa)
    A_Sero=float(options.A_Sero)
    #Reward difference for dopamine and serotonin
    R_D=float(options.R_D)
    #Time for tonic dopamine to be active (in ms)
    T_R=int(options.T_R)
    #activation and inhibition of serotonergic pathway
    Activ=options.Activ
    Inhib=options.Inhib
    tau_DA=float(options.TDA) #time constant pre-post window
    tau_Sero=float(options.TSero)   #time constant post-pre window for serotonin
    #Random locations
    randomloc = options.randomloc
    tau_e_sero = float(options.tau_e_sero)*10**3 #Elegibility trace of 5HT
    if randomloc:
        np.random.seed(100)
        i_pos = np.random.rand(episodes,2)
    ...
    if episodes==1:
        ...
        descriptor = {'jobID':jobID,'Trials':Trials,'episodes':episodes,'eta':eta,'A_DA':A_DA,'A_Sero':A_Sero,'Activ':Activ,'Inhib':Inhib,'R_D':R_D,'T_R':T_R,'T_DA':tau_DA,'T_Sero':tau_Sero,'tau_e_sero':tau_e_sero}
...
def episode_run(jobID,episode,plot_flag,Trials,changepos,Sero,eta,A_DA,A_Sero,Activ,Inhib,R_D,T_R,tau_DA,tau_Sero,initial_pos,tau_e_sero,):
        ...
        avg_weight = np.zeros(Trials)
        w_0_change = np.zeros(Trials)
        w_108_change = np.zeros(Trials)
        std_weight = np.zeros(Trials)
        cv_weight = np.zeros(Trials)
        ...
        starting_position = np.array(initial_pos) #starting position
        t_rew = T_max #time of reward - initialized to maximum
        t_extreme = t_rew+300 #time of reward - initialized to maximum
        t_end = T_max
        ...
        A_post_pre_sero=A_Sero  #amplitude post-pre window for serotonin
        ...
        tau_e_sero= tau_e_sero #time constant eligibility trace for serotonin
        ...
        while i<T_max*Trials:
            ...
            #online weights update STDP with unsymmetric window and depression due to serotonin w tonic Delta normalised to the total reard
            if Sero:
                if not(Inhib) and not(Activ):
                    ...
                    w_tot[0:N_action,0:N_pc] = w_tot[0:N_action,0:N_pc]+eta*(-eligibility_trace_sero*(1-rew_found)+R_D*rew_found*eligibility_trace)
                elif Activ and not any(lower <= i<= upper for (lower, upper) in ranges):
                    #If there is overpotentiation of serotonin, assumed as doubled
                    trace_pre_post_sero, trace_post_pre_sero,eligibility_trace_sero, trace_tot_sero, W_sero = weights_update_stdp(1.5*A_pre_post_sero, 1.5*A_post_pre_sero, tau_pre_post_sero, tau_post_pre_sero, np.matlib.repmat(X.T,N_action,1) , np.matlib.repmat(Y_action_neurons,1, N_pc), trace_pre_post_sero, trace_post_pre_sero, trace_tot_sero, tau_e_sero)
                    w_tot[0:N_action,0:N_pc]= w_tot[0:N_action,0:N_pc]+eta*(-R_activ*eligibility_trace_sero*(1-rew_found)+R_D*rew_found*eligibility_trace)
                elif Inhib and not any(lower <= i<= upper for (lower, upper) in ranges):
                    #If there is inhibition of serotonin, no eligibility trace is produced
                    w_tot[0:N_action,0:N_pc]= w_tot[0:N_action,0:N_pc]+eta*R_D*rew_found*eligibility_trace
                else:
                    #For parts of optogenetic stimulation
                    trace_pre_post_sero, trace_post_pre_sero,eligibility_trace_sero, trace_tot_sero, W_sero = weights_update_stdp(A_pre_post_sero, A_post_pre_sero, tau_pre_post_sero, tau_post_pre_sero, np.matlib.repmat(X.T,N_action,1) , np.matlib.repmat(Y_action_neurons,1, N_pc), trace_pre_post_sero, trace_post_pre_sero, trace_tot_sero, tau_e_sero)
                    w_tot[0:N_action,0:N_pc] = w_tot[0:N_action,0:N_pc]+eta*(-eligibility_trace_sero*(1-rew_found)+R_D*rew_found*eligibility_trace)
            else:
                w_tot[0:N_action,0:N_pc]= w_tot[0:N_action,0:N_pc]+eta*R_D*rew_found*eligibility_trace
            ...
            if pos[0]<=bounds_x[0]:
                ...
                if pos[0]>= bounds_x[1]:
                    ...
                    if pos[1]<=bounds_y[0]:
                        ...
                        if pos[1]>=bounds_y[1]:
                            pos = pos+dx*np.array([0,-1])
            #time when trial end is T_R ms after reward is found
            t_extreme = t_rew+T_R
            ...
            if t==0:
                ...
                #store weights and actions at half and end of the episode for representation
                if (tr==(Trials/2)+1):
                    w_values_h =  np.mean(w_tot[:,0:N_pc],axis=0)
                    pi_values_h = ac.T/np.max(np.linalg.norm(ac,axis=0))
                elif (tr==Trials):
                    w_values_f = np.mean(w_tot[:,0:N_pc],axis=0)
                    pi_values_f = ac.T/np.max(np.linalg.norm(ac,axis=0))

                ## save quadrant
                quadrant_map[tr-1,:]=quadrant.flatten()/np.sum(quadrant)
                avg_weight[tr-1] = np.mean(np.mean(w_tot[:,0:N_pc],axis=0))
                w_0_change[tr-1] = np.mean(w_tot[:,0],axis=0)
                w_108_change[tr-1] = np.mean(w_tot[:,108],axis=0)
                std_weight[tr-1] = np.std(w_tot[:,0:N_pc].flatten())
                cv_weight[tr-1] = std_weight[tr-1]/avg_weight[tr-1]
        ...
        return episode,first_reward,rewarding_trials,punishing_trials,quadrant_map,median_distance,time_reward,time_reward2,time_reward_old, cv_weight,w_108_change,w_0_change
