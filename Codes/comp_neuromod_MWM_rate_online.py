...
"""
Competitive weight change (CWC) with rate-based implementation.
...
Run as: python comp_neuromod_MWM_rate_online.py -o <name_job> -e <episodes> -t <trials> -s
...
def main():
    ...
    parser.add_option("-e", "--episodes", dest="episodes",
          help="Runs to be performed", metavar="int")
...
def episode_run(jobID,episode,plot_flag,Trials,changepos,Sero,eta,A_DA,A_Sero,Activ,Inhib,R_D,T_R,tau_DA,tau_Sero,initial_pos,tau_e_sero,):
        ...
        while i<T_max*Trials:
            ...
            if Sero:
                if not(Inhib) and not(Activ):
                    ...
                    W1_sero, eligibility_trace_sero, trace_tot_sero, W_sero = weights_update_rate(1.5*(A_pre_post_sero+A_post_pre_sero)/2, tau_pre_post_sero, np.matlib.repmat(prob,N_action,1), np.matlib.repmat(np.squeeze(rho_action_neurons),N_pc,1).T, W1_sero, trace_tot_sero, tau_e_sero)
        ...
        return episode,first_reward,rewarding_trials,punishing_trials,quadrant_map,median_distance,time_reward,time_reward2,time_reward_old, avg_weight,pos_hist
