...
"""
Code for forward and reversal learning paradigm with single punishing and rewarding events (Figure 4)
...
Options:
    ...
    - Reversal. Swicth locations of reward and punishment middle episode of a trial (-c --changeposition).
...
Run as: python RL_plasticity_reversal.py -o <name_job> -e <episodes> -t <trials> -s
...
def main():
    ...
    parser.add_option("-e", "--episodes", dest="episodes",
          help="Runs to be performed", metavar="int",default=40)
    ...
    if plot_flag:
        ...
        plt.close()
    if episodes==1:
        episode_run(jobID,1,plot_flag,Trials,changepos,Sero,eta_DA,eta_Sero,A_DA,A_Sero,)
    else:
        ...
        descriptor = {'jobID':jobID,'Trials':Trials,'episodes':episodes,'eta_DA':eta_DA,'eta_Sero':eta_Sero,'A_DA':A_DA,'A_Sero':A_Sero}
...
def episode_run(jobID,episode,plot_flag,Trials,changepos,Sero,eta_DA,eta_Sero,A_DA,A_Sero,):
        ...
        c2 = np.array([-1.5,-1.5]) #centre negative reward 1
        r_goal2 = 0.3 #radius goal area 3
        ...
        tau_pre_post_sero= 10   #time constant pre-post window for serotonin
        tau_post_pre_sero= 10   #time constant post-pre window for serotonin
        tau_e_sero= 2*10**3 #time constant eligibility trace for serotonin
        ...
        time_reward_neg= np.copy(time_reward) #store time when agent enters negative reward
        time_reward_neg_old= np.copy(time_reward) #store time when agent enters the old negative reward
        time_reward_neg2 = np.copy(time_reward) #stores time of negative reward 2 (moved)
        ...
        if plot_flag:
            ...
            reward_plot = ax1.plot(c[0]+r_goal*np.cos(np.linspace(-np.pi,np.pi,100)), c[1]+r_goal*np.sin(np.linspace(-np.pi,np.pi,100))) #plot reward 1
            point_plot,= ax1.plot(starting_position[0],starting_position[1], 'r',marker='o',markersize=5) #plot initial starting point
            punish_plot = ax1.fill(c2[0]+r_goal2*np.cos(np.linspace(-np.pi,np.pi,100)), c2[1]+r_goal2*np.sin(np.linspace(-np.pi,np.pi,100)),"r") #plot negative reward
        ...
        while i<T_max*Trials:
            ...
            if t==1:
                ...
                rew_n_found=0 #flag that negative reward is found
                ...
                if (tr==(Trials/2)+1) and changepos:
                    rew1_flag=0
                    rew2_flag=1
                    w_tot_old_2=w_tot_old
                    np.linspace(-np.pi, np.pi, 100)
                    if plot_flag:
                        reward_plot.pop(0).remove()
                        punish_plot.pop(0).remove()
                        reward_plot = ax1.plot(c2[0]+r_goal2*np.cos(np.linspace(-np.pi,np.pi,100)), c2[1]+r_goal2*np.sin(np.linspace(-np.pi,np.pi,100))) #plot negative reward 2
                        punish_plot = ax1.fill(c[0]+r_goal*np.cos(np.linspace(-np.pi,np.pi,100)), c[1]+r_goal*np.sin(np.linspace(-np.pi,np.pi,100)),"r") #plot reward 2
            ## place cells
            ...
            if np.sum((pos-c)**2)<=r_goal**2 and rew_found==0 and rew_n_found==0 and rew1_flag==1:
            ...
            #negative reward 1 for the fist half of the trial
            if np.sum((pos-c2)**2)<=r_goal2**2 and rew_n_found==0 and Sero and rew1_flag==1:
                time_reward_neg[tr-1]=t #store time of entrance old reward location
                t_rew=t
                punishing_trials[tr-1:]+=1
                rew_n_found=1 #change reward for weight modification

            #cases for location switching

            # agent enters reward 2 in the second half of the trial
            if np.sum((pos-c2)**2)<=r_goal2**2 and rew_found==0 and rew_n_found==0 and rew2_flag==1:
            ...
            #negative reward 2 for the second half of the trial
            if np.sum((pos-c)**2)<=r_goal**2 and rew_n_found==0 and Sero and rew2_flag==1:
                rew_n_found=1 #change reward for weight modification
                t_rew=t #time of negative reward 2
                time_reward_neg2[tr-1] = t #store time of reward 2
                punishing_trials[tr-1:]+=1
            ...
            # agent enters negative reward 1 in the second half of the trial (previously rewarded location)
            if np.sum((pos-c2)**2)<=r_goal2**2 and rew1_flag==0 and rew2_flag==1:
                 #this location is no longer punished, so the trial is not ended
                time_reward_neg_old[tr-1]=t #store time of entrance old negative reward location
            ...
            if t==0:
                ...
                # if the negative reward is found, weights are retroactively depressed through an eligibility trace
                if Sero:
                    if rew_n_found!=0:
                        w_tot[0:N_action,0:N_pc]= w_tot[0:N_action,0:N_pc]*(1-rew_n_found) + (w_tot_old-eta_Sero*eligibility_trace_sero)*rew_n_found
                ...
                #store weights at half and end of the episode for representation
                if (tr==(Trials/2)+1):
                    w_values_h =  np.mean(w_tot[:,0:N_pc],axis=0)
                    pi_values_h = ac.T/ac_norm
                elif (tr==Trials+1):
                    w_values_f = np.mean(w_tot[:,0:N_pc],axis=0)
                    pi_values_f = ac.T/ac_norm
        ...
        return episode,first_reward,rewarding_trials,punishing_trials,quadrant_map,median_distance,time_reward,time_reward_neg,time_reward2,time_reward_neg2,w_tot_old,avg_weight,w_values_h,pi_values_h,w_values_f,pi_values_f
