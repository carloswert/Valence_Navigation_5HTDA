...
"""
Sequential weight change (SWC) implementation with a constant 5-HT function instead of Delta.
...
Run as: python seq_neuromod_MWM_constant.py -o <name_job> -e <episodes> -t <trials> -s
...
def main():
    ...
    parser.add_option("-t", "--trials", dest="trials",
      help="Number of trials", metavar="int",default=1000)
    ...
    parser.add_option("-m", "--adopa", dest="A_Dopa",
    ...
    parser.add_option("-y", "--elegib", dest="tau_e_sero",
    ...
    if episodes==1:
        initial_pos = [0,0]
        episode_run(jobID,1,plot_flag,Trials,changepos,Sero,eta_DA,eta_Sero,A_DA,A_Sero,Activ,Inhib,tau_DA,tau_Sero,initial_pos,tau_e_sero,)
    else:
        ...
        for episode in range(0,episodes):
            ...
            if randomloc:
                ...
                initial_pos = [0,0]
            results.append(pool.apply_async(episode_run,(jobID,episode,plot_flag,Trials,changepos,Sero,eta_DA,eta_Sero,A_DA,A_Sero,Activ,Inhib,tau_DA,tau_Sero,initial_pos,tau_e_sero,),error_callback=log_e))
        ...
        descriptor = {'jobID':jobID,'Trials':Trials,'episodes':episodes,'eta_DA':eta_DA,'eta_Sero':eta_Sero,'A_DA':A_DA,'A_Sero':A_Sero,'Activ':Activ,'Inhib':Inhib,'tau_e_sero':tau_e_sero}
...
def episode_run(jobID,episode,plot_flag,Trials,changepos,Sero,eta_DA,eta_Sero,A_DA,A_Sero,Activ,Inhib,tau_DA,tau_Sero,initial_pos,tau_e_sero,):
        ...
        while i<T_max*Trials:
            ...
            if Sero:
                if not(Inhib) and not(Activ):
                    ...
                    w_tot[0:N_action,0:N_pc]= w_tot[0:N_action,0:N_pc]-eta_Sero*eligibility_trace_sero*(1/T_max)
                    ...
                    trace_pre_post_sero, trace_post_pre_sero,eligibility_trace_sero, trace_tot_sero, W_sero = weights_update_stdp(2*A_pre_post_sero, 2*A_post_pre_sero, tau_pre_post_sero, tau_post_pre_sero, np.matlib.repmat(X.T,N_action,1) , np.matlib.repmat(Y_action_neurons,1, N_pc), trace_pre_post_sero, trace_post_pre_sero, trace_tot_sero, tau_e_sero)
                    w_tot[0:N_action,0:N_pc]= w_tot[0:N_action,0:N_pc]-eta_Sero*eligibility_trace_sero*(1/T_max)
